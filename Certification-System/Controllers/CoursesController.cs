    ...
    public class CoursesController : Controller
    {
        ...
        public ActionResult DeleteUserFromCourseQueue(string userIdentificator, string courseIdentificator, bool userAssignedToCourse)
        {
            ...
            if (courseQueue != null && user != null)
            {
                ...
                {
                    return RedirectToAction("AdminNotificationManager", "Notifications", new { message = "Zgłoszenie pracownika zostało przyjęte." });
                }
                else
                {
                    var course = _context.courseRepository.GetCourseById(courseIdentificator);
                    ...
                    return RedirectToAction("AdminNotificationManager", "Notifications", new { message = "Zgłoszenie pracownika zostało odrzucone." });
                }
            }
        ...
        // GET: AssignCompanyWorkersToCourse
        [Authorize(Roles = "Company")]
        public ActionResult AssignCompanyWorkersToCourse(string courseIdentificator)
        {
            if (string.IsNullOrWhiteSpace(courseIdentificator))
            {
                return RedirectToAction("BlankMenu", "Certificates");
            ...
            if (course.DateOfStart > DateTime.Now && !course.CourseEnded)
            {
                var companyWorkersNotEnrolledInCourse = companyWorkers.Where(z => !z.Courses.Contains(courseIdentificator)).ToList();
        ...
        public async Task<ActionResult> AssignCompanyWorkersToCourse(AssignCompanyWorkersToCourseViewModel addCompanyWorkersToCourseViewModel)
        {
            if (ModelState.IsValid)
            {
                if (addCompanyWorkersToCourseViewModel.Course.DateOfStart > DateTime.Now)
                {
                    ...
                    ModelState.AddModelError("", $"Do kursu można dodać maksymalnie {addCompanyWorkersToCourseViewModel.VacantSeats} użytkowników");
